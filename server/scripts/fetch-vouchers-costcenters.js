...
/**
 * Fetch vouchers from Tally and output JSON including narration and cost centres.
...
// Collect and join all string leaves within an object/array
const deepStringify = (value, maxLen = 500) => {
    ...
    if (typeof value === 'object') {
        ...
        // Common Tally narration/text shapes
        if (value['TEXT']) {
            const t = value['TEXT'];
...
const extractNarration = (voucher) => {
    ...
    // Avoid placeholder strings like 'String' or [object Object]
...
const getVouchersPayload = (fromDate = null, toDate = null) => {
    const formatYmd = (d) => {
...
const normalizeVouchers = (parsed) => {
    ...
    return vouchers
        ...
        .filter((v) => v.voucherType.toLowerCase() === 'sales');
};
...
(async () => {
    try {
        ...
        console.log(`🚀 Fetching vouchers ${useOffline ? '(offline fallback)' : 'from Tally'}...`);
        const xmlPayload = getVouchersPayload();
        let parsed = null;
        if (useOffline) {
        ...
        const out = {
            metadata: {
                ...
                tallyUrl: TALLY_URL,
                description: 'Sales vouchers with narration and cost centres extracted directly from Tally',
                summary: {
                    ...
                    uniqueParties: Array.from(new Set(vouchers.map(v => v.party).filter(Boolean))).sort(),
                },
            },
            vouchers,
        };

        const outPath = path.join(__dirname, 'sales-vouchers-with-costcenters.json');
        ...
        console.log(JSON.stringify({
            ...
            sample: vouchersWithCC.slice(0, 3)
        }, null, 2));
